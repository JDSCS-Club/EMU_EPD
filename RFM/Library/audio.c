...
//========================================================================
{
	if ( GetAudioIC() != AudioXE3005 )	return;

	//uint8_t		nBuf[10];
	uint16_t	nData;

	//	nBuf[1] = (WRITE_REG | addr);
	//	nBuf[0] = nData & 0xFF;

	nData = ( ( ( WRITE_REG | addr ) << 8 ) | ( data & 0xFF ) );

	//	spi_write( 2, &nBuf, 1 );
	spi_write( 2, (uint8_t *)&nData, 1 );
}
...
/***************************************************************************
Declaration : void InitCodecXE3005(void)
...
void InitCodecXE3005	( void )
{
	...
	SetAudioIC( AudioXE3005 );

	//	return;

		/*
			200103_v1.0.0.90.hex -> SDO Ok
				spi_write(141) : 0xF082
				spi_write(141) : 0x8C84
				spi_write(141) : 0x0188
				spi_write(141) : 0x2489
	...
	WriteSPICodec( C_REG, 0xF0 );				// Fsync ( 8 KHz ) < 20kHz 

	WriteSPICodec( E_REG, 0x8C );//0b10001100 );			// Enable VMIC(VREG11), Gain = 20x

	//	DIV : 8020 KHz / 4
//	WriteSPICodec( I_REG, 0x03 );//0b00000001 );			// OK

	//	DIV : 2048 KHz / 1
	WriteSPICodec( I_REG, 0x00 );//0b00000001 );			// OK

	//	Codec ( Master ) Mode
//	WriteSPICodec( J_REG, 0x34 );// 0b00110100 );	// Master Mode using 1'st channel

	//*
	WriteSPICodec( J_REG, 0x25 );// 0b00100101 );	// Slave Mode / SFS
	...
	//	Loopback
	WriteSPICodec( J_REG, 0xA5 );		//    Audio Loopback Test
...
//	WriteSPICodec( E_REG, 0x08 );//0b00001100 );			// Disable VMIC(VREG11), Gain = 20x

	WriteSPICodec( I_REG, 0x01 );//0b00000001 );			// OK
//	WriteSPICodec( I_REG, 0x00 );//0b00000001 );			// OK

	WriteSPICodec( J_REG, 0x24 );// 0b00100100 );			// Slave Mode using 1'st channel
	...
	//	Analog input
//	WriteSPICodec( E_REG, 0x0C );// 0b00001100 );			// Disable VMIC, Gain = 20x
	WriteSPICodec( E_REG, 0x8C );//0b10001100 );			// Enable VMIC(VREG11), Gain = 20x

	//	Block on/off clock division
//	WriteSPICodec( I_REG, 0x0D );// 0b00001101 );			// Disable DA, PA and AD

	//	MCLKDIV 00 : 1
//	WriteSPICodec( I_REG, 0x00 );//0b00000001 );			// Fail
	WriteSPICodec( I_REG, 0x01 );//0b00000001 );			// OK
//	WriteSPICodec( I_REG, 0x03 );//0b00000001 );			// Enable DA, PA and AD
	...
	//    Audio Loopback Test
	//	spi.w wr 2 0x8880
	WriteSPICodec( J_REG, 0x80 );		//    Audio Loopback Test
	//	*/
...
//========================================================================
{
	printf( "%s(%d) - %d\n", __func__, __LINE__, bMute );

	if ( bMute )
	{
		//	Spk Mute On
		WriteSPICodec( I_REG, 0x08 );	//	[ 0000 | EN_DAC[3] | EN_ADC[2] | MCLKDIV[1:0] ]
	}
	else
	{
		//	Spk Mute Off
		WriteSPICodec( I_REG, 0x00 );	//	[ 0000 | EN_DAC[3] | EN_ADC[2] | MCLKDIV[1:0] ]
	}
}
...
//========================================================================
{
	...
	{
		//	Spk Mute On
		WriteSPICodec( I_REG, 0x04 );	//	[ 0000 | EN_DAC[3] | EN_ADC[2] | MCLKDIV[1:0] ]
	}
	else
	{
		//	Spk Mute Off
...
//========================================================================
{
	...
	if ( bOnOff )
	{
		WriteSPICodec( J_REG, 0xA5 );		//	Loopback[7] / reserved[5] / TX_FIRST[2] / SFS[0]
	}
	else
	{
		WriteSPICodec( J_REG, 0x25 );		// reserved[5] / TX_FIRST[2] / SFS[0]
	}
}
